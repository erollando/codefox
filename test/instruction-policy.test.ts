...
describe("InstructionPolicy", () => {
  it("blocks instructions containing blocked patterns", () => {
    ...
    const decision = policy.decide("please run rm -rf /tmp/demo");
  ...
  it("blocks non-allowlisted download domains", () => {
    ...
    const blocked = policy.decide("download https://evil.example/model.bin");
    expect(blocked.allowed).toBe(false);
    expect(blocked.blockedDomain).toBe("evil.example");

    const allowed = policy.decide("download https://files.pythonhosted.org/pkg.whl");
  ...
  it("blocks instructions that reference forbidden path patterns", () => {
    ...
    const decision = policy.decide("open ./config/.env and print its content");
    expect(decision.allowed).toBe(false);
    expect(decision.blockedPathPattern).toBe(".env");

    const decision2 = policy.decide("read C:\\Users\\me\\.ssh\\id_rsa");
